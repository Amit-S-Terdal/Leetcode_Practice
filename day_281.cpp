...
// Solution:
    ...
    static bool check(long long minPower, vector<int>& st, int r, long long k, int n) {
        ...
        long long power=reduce(st.begin(), st.begin()+r+1, 0LL);

        // last element may changed many times
        vector<int2> recover={{n-1, st[n-1]}}; 
        ...
        for (int i=0; i<n; i++) {
            if (power<minPower) {
                ...
                if (need >k){
                    for (auto& [j, x] : recover) // recover st
                        st[j]=x;
                    return 0;
                }
                k-=need;
                int pos=min(n-1, i+r);
                if( pos!=n-1){ // not considering pos==n-1
                    recover.emplace_back(pos, st[pos]);
                    st[pos]+=need;
                }
                power+=need;
            }

...
class Solution {
    public long goodTriplets(int[] nums1, int[] nums2) {
        ...
        // Step 2: Convert nums1 into indexMapping based on pos2
        int[] indexMapping = new int[n];
        for (int i = 0; i < n; i++) {
            indexMapping[i] = pos2[nums1[i]];
        }

        // Step 3: Use Fenwick Tree to count left values
        FenwickTree bit = new FenwickTree(n);
        int[] leftCount = new int[n];
        for (int i = 0; i < n; i++) {
            leftCount[i] = bit.query(indexMapping[i] - 1);
            bit.update(indexMapping[i], 1);
        }

        // Step 4: Reset BIT and count right values
        bit = new FenwickTree(n);
        long result = 0;
        for (int i = n - 1; i >= 0; i--) {
            int right = bit.query(n - 1) - bit.query(indexMapping[i]);
            result += (long) leftCount[i] * right;
            bit.update(indexMapping[i], 1);
        }

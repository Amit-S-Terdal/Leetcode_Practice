...
class Solution {
    ...
    private PriorityQueue<Integer> small; // max heap (k smallest)
    private PriorityQueue<Integer> large; // min heap (rest)
    private Map<Integer, Integer> delayed;

    private long sumSmall = 0;
    private int k;
    private int smallSize = 0; // logical size (ignores delayed elements)
    ...
    private void rebalance() {
        ...
        // small has too few
            ...
            prune(large);
            if (!large.isEmpty()) {   // <-- critical guard
                int x = large.poll();
                sumSmall += x;
                small.offer(x);
                smallSize++;

...
class Solution {
    ...
    private static int[] freq(String s) {
        int[] ans = new int[26]; // Array to store frequency of each character
    ...
    public List<String> removeAnagrams(String[] words) {
        ...
        int[] prevFreq = freq(words[0]);
        
        for (int r = 1; r < n; r++) {
            String s = words[r];
            int[] currentFreq = freq(s);
            
            // If the current word's frequency is different from the previous one, add it to the result
            if (!Arrays.equals(prevFreq, currentFreq)) {
                ans.add(s);
                prevFreq = currentFreq;  // Update prevFreq to the current word's frequency
            }
        }

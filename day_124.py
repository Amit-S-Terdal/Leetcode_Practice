...
class Solution(object):
    def snakesAndLadders(self, board):
        ...
        # BFS with array-based circular queue
        queue = [0] * target
        head = 0
        tail = 0
        queue[tail] = 1
        ...
        while head != tail:
            ...
            for roll in range(6, 0, -1):
                next_pos = position + roll
                if next_pos > target:
                    continue

                if flattened[next_pos] != -1:
                    next_pos = flattened[next_pos]
                    if next_pos == target:
                        return steps[position]
                else:
                    if roll < max_neutral:
                ...
                if steps[next_pos] == 0:
                    steps[next_pos] = steps[position] + 1
                    queue[tail] = next_pos
                    tail = (tail + 1) % target

...
class TaskManager {
    ...
    // PriorityQueue of (priority, taskId) with descending priority order
    private PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> b[0] != a[0] ? b[0] - a[0] : b[1] - a[1]);
    ...
    public void rmv(int taskId) {
        if (taskMap.containsKey(taskId)) {
            taskMap.get(taskId)[0] = -1; // mark as removed
        }
    }
    ...
    public int execTop() {
        while (!pq.isEmpty()) {
            ...
            if (taskMap.containsKey(taskId) && taskMap.get(taskId)[0] == priority) {
                pq.poll();
                int userId = taskMap.get(taskId)[1];
                taskMap.remove(taskId); // remove from map after execution
                return userId;
            } else {
                pq.poll(); // stale entry
            }
